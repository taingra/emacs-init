...
I've been a longtime user of the [[https://www.musicpd.org/][Music Player Daemon]] (MPD).  However,
I've only recently started trying to add a Emacs client.  In the past
I had tried the client included with Emacs mpc.el but I found it's
organization too opinionated as I never really liked rhythmbox which
it bases it's UI on.

I was thinking about hacking on it to make a more dired-like UI, then
I discovered someone else had the same idea.  mpdired is a client
written from scratch, so there no code-reuse from mpc.el.  That means
