...
Source files available at [[https://git.sr.ht/~taingram/emacs-init][git.sr.ht/~taingram/emacs-init]]
#+end_notice
...
I had held out for many years on creating a literate config, but
seeing other users' success convinced me.  The ability to maintain
complete notes, links, and the configuration code all in one file is
extremely convenient.  In addition, the hierarchical structure of Org
allows for a clearer organization of the code.
...
The complete Org file is available on [[https://git.sr.ht/~taingram/emacs-init][sourcehut]].  However, I would not
recommend copying this configuration directly as it is tailored for my
particular usage.  Instead I recommend skimming through this article,
copy the sections that interest you, and adjust them to your
preferences.  Emacs is /your/ editor and if it does not reflect your
preferences then you are not using it correctly.
...
*** use-package --- Macro for Installing and Configuring Packages
...
The [[info:emacs#Fetching Package Sources][:vc keyword has been added to use-package in Emacs 30]].  This
should make it easy to add packages outside the default archives.
This is probably safer than using MELPA since you have to explicitly
acknowledge you are installing an external package and verifying the
repository source.

*** Hide minor modes in the mode-line

There are two different packages for hiding or modifying how
minor modes are shown on the mode-line.

 - [[https://savannah.nongnu.org/projects/delight][delight.el]]
...
Taken from [[https://www.reddit.com/r/emacs/comments/idz35e/emacs_27_can_take_svg_screenshots_of_itself/][oantolin on reddit]], really neat feature.
...
is keeping consistent bindings across modes as possible, may modes try
to do this by default to varying levels of success.

I tried which-key but I find the output hard to visually parse.  I
prefer to use the existing help options.
...
** Repeat-mode --- Repeat Commands Quickly
...
*** Vertico --- Vertical Completions in Mini-Buffer
...
*** Orderless --- Orderless Fuzzy Searching
...
#+begin_src emacs-lisp
  (use-package dired
    ...
    ;; Easy dual-pane file manager
    (setq dired-dwim-target t)

    (setq dired-free-space nil)
    (setq dired-listing-switches "-lahG --group-directories-first --time-style=long-iso")
...
[[info:emacs#Rmail][rmail]] is also built into Emacs but only supports the mbox mail format
which is not widely used by mail providers.
...
I've been a longtime user of the [[https://www.musicpd.org/][Music Player Daemon]] (MPD).  I
generally used ncmpcpp to control it, although I find myself wanting
an easy way to control music from Emacs.  Ideally an easy way to
add/remove songs from the queue using a dired-like view.

I previously tried the included mpd client included with Emacs mpc.el.
I disliked the rhythmbox-inspired UI, poor playlist support, and
multiple windows.  I tend to dislike any Emacs package that starts
messing with my window setup unless absolutely necessary.

I was thinking about hacking it to make a more dired-like UI, then I
discovered someone else already had the same idea.  mpdired is a
client written from scratch, so there no code-reuse from mpc.el.  It
basically works exactly how I was hoping.  I just wish it was a little
more interactive so I'm hoping to add some enhancements.

My fork can be found on sourcehut [[https://mpdired-highlight-progress][git.sr.ht/~taingram/mpdired/]]

#+begin_src emacs-lisp
    (unless (eq window-system 'android)
      (use-package mpdired
        :vc (:url  "https://git.sr.ht/~taingram/mpdired"
    	 :rev :newest)
        :hook (mpdired-mode . hl-line-mode)
        :bind (:map mpdired-mode-map
  	     ("b" . mpdired-toggle-view))
        :config
        (setq mpdired-highlight-progress nil)))
#+end_src

I am also comparing mpdired with the more feature rich [[https://www.gnu.org/software/emms/][emms package]].
The downside of the emms package is it is not completely integrated
with mpd.  It can be used to play ANY type of media in a playlist
...
*** Eglot ---Language Server Protocol (LSP)
...
*** Corfu ---Drop down auto-completion support
...
*** Flymake --- Error Checking in Buffer
...
*** Tempo --- Code Templates / Snippets

I have recently removed yasnippets from my config as I rarely used the
snippets and often forgot the keywords.  When I started using
yasnippets I downloaded a repo of premade snippet definitions so that
meant I had a lot of stuff I never used and often triggered
accidentally.  I have started porting my few snippets to tempo.el
which is built into Emacs.
...
Tempo is a really cool package built into Emacs that is not well
documented and does not appear to have a lot of users.  Most people
seem to use yasnippets instead.
...
can be enabled by calling the function in an abbrev definition:
...
Paredit is a really cool way to do structural editing on
s-expressions.  They keybindings can be a little confusing but it is
really helpful to learn and use.
...
See [[https://taingram.org/blog/emacs-javascript-lsp.html][my post]] for setting up a JavaScript LSP server.
...
#+begin_src emacs-lisp
  ...
  (unless (eq window-system 'android)
    (use-package gd-script-mode
      ...
      (setq gdscript-docs-local-path "/home/tom/Documents/References/godot4.3-docs")))
#+end_src
...
#+begin_src emacs-lisp
  (use-package org
    ...
    (require 'org-tempo)
    (tempo-define-template "org-elisp-src-block"
                           '("#+begin_src emacs-lisp" n p n "#+end_src" n)
                           "<el"

                       "Insert an Emacs Lisp source code block template")
    ;; Alternative tag abbreviation
    (tempo-add-tag "<es" #'tempo-template-org-elisp-src-block)
    ...
    (defun tai-org-capture-todo ()
      (interactive)
      (org-capture nil "t"))

    (setq org-latex-title-command "\\begin{center}
    %s \\hfill %D
    \\smallskip
    \\hrule
    \\bigskip
    {\\LARGE \\textbf{\\MakeUppercase{%t}}\\par}
    \\smallskip
    %a
  \\end{center}")
    ...
    (org-capture-templates
     `(("t" "Todo" entry
       ...
       ("w" "Writing Idea" entry
        ...
        "* IDEA %^{Writing Idea}\n%u\n")
       ("i" "Idea" entry (file+headline "~/todo.org" "Project Idea")
        "* IDEA %^{Project Idea}\n%u\n")
    ...
    (org-agenda-custom-commands
     '(("h" "General Hobby Tasks"
        ((agenda ""
         ...
         (tags-todo "-PROJECT-IDEA-HOLD/TODO|+LEVEL=2+PROJECT/TODO"
  		  ((org-agenda-overriding-header "Tasks: TODO")))
         (tags-todo "-PROJECT-IDEA/HOLD|+LEVEL=2+PROJECT/HOLD"
  		  ((org-agenda-overriding-header "Tasks on Hold: HOLD"))))
